...
This framework calculates and stores "intersection efficiencies" for the application of the "inclusion method" [1], to be used for the HH\rightarrow bbττ resonant analysis by the CMS experiment. More details available in [[https://indico.cern.ch/event/1143576/#13-trigger-sf-update][this presentation]].

The processing starts from skimmed ([[https://github.com/LLRCMS/KLUBAnalysis][KLUB]]) Ntuples. The framework is managed by [[https://github.com/spotify/luigi][luigi]] (see ~run.py~), which runs local tasks and creates a [[https://htcondor.readthedocs.io/en/latest/index.html][HTCondor]] Direct Acyclic Graph (DAG).
...
#+BEGIN_SRC shell
python3 sf_inclusion/run.py --tag FullWorkflowTest --data MET EGamma --mc_process TT --configuration sel_default
#+END_SRC
...
Install dependencies in a separate conda environment, including:
#+BEGIN_SRC shell
conda install h5py
