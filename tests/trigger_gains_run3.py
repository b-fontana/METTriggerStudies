...
def main(args):
    ...
    file70 = uproot.open("data/regions_preEE_11p2_all.root")
    file60 = uproot.open("data/regions_preEE_11p3-jet-lower-pt_all.root")
    file80 = uproot.open("data/regions_preEE_11p4-jet-higher-pt_all.root")
    # petroff6 = ["#5790fc", "#f89c20", "#e42536", "#964a8b", "#9c9ca1", "#7a21dd"]

    # channel_base_60 = file60['Base{}_baseline_{}_genHH_mass'.format(base_extension, args.channels[0])]
    # channel_base_70 = file70['Base{}_baseline_{}_genHH_mass'.format(base_extension, args.channels[0])]
    # channel_base_80 = file80['Base{}_baseline_{}_genHH_mass'.format(base_extension, args.channels[0])]
    # channel_base_plus_ttjet_60 = file60['Base{}ORTauTauJet_baseline_{}_genHH_mass'.format(base_extension, args.channels[0])]
    # channel_base_plus_ttjet_70 = file70['Base{}ORTauTauJet_baseline_{}_genHH_mass'.format(base_extension, args.channels[0])]
    # channel_base_plus_ttjet_80 = file80['Base{}ORTauTauJet_baseline_{}_genHH_mass'.format(base_extension, args.channels[0])]
    # channel_all = file60['Base{}ORTauTauJetOR4JetsPNet_baseline_{}_genHH_mass'.format(base_extension, args.channels[0])]
    # err_base_plus_ttjet_60 = np.sqrt(channel_base_plus_ttjet_60.values())/ channel_all.values()
    # err_base_plus_ttjet_70 = np.sqrt(channel_base_plus_ttjet_70.values())/ channel_all.values()
    # err_base_plus_ttjet_80 = np.sqrt(channel_base_plus_ttjet_80.values())/ channel_all.values()
    # ax = channel_base_60.axes[0].centers()    
    # edges = channel_base_60.axes[0].edges()
    # xerr_r = ax - edges[:-1]
    # xerr_l = edges[1:] - ax
    # hep.style.use("CMS")
    # print("CMS style")
    ...
    # ax2.bar(ax, height=channel_all.values(), width=xerr_r * 2, color=petroff6[4], alpha=0.3, zorder=0, label="HHbbtautau")
    # print(channel_base_60.values(), channel_base_70.values(), channel_base_80.values())
    # hep.style.use("CMS")
    # ax1.errorbar(ax - 10, (channel_base_plus_ttjet_60.values() / channel_base_60.values() - 1) * 100, yerr=err_base_plus_ttjet_60*100, xerr=[xerr_r - 10, xerr_l + 10], fmt='o', label=r"p$_{t}^{jet} > 60$ GeV", color=petroff6[0], zorder=5)
    # ax1.errorbar(ax, (channel_base_plus_ttjet_70.values() / channel_base_60.values() - 1) * 100, yerr=err_base_plus_ttjet_70*100, xerr=[xerr_r, xerr_l], fmt='o', label=r"p$_{t}^{jet} > 70$ GeV", color=petroff6[2], zorder=5)
    # ax1.errorbar(ax + 10, (channel_base_plus_ttjet_80.values() / channel_base_60.values() - 1) * 100, yerr=err_base_plus_ttjet_80*100, xerr=[xerr_r + 10, xerr_l - 10], fmt='o', label=r"p$_{t}^{jet} > 80$ GeV", color=petroff6[4], zorder=5)
    ...
    # plt.savefig("cutoff_comparison.png".format(args.channels[0]))
    with uproot.open("data/regions_preEE_12p11-old-way-pf75_all.root") as file:
        # from matplotlib.colors import ListedColormap
        petroff6 = ["#5790fc", "#f89c20", "#e42536", "#964a8b", "#9c9ca1", "#7a21dd"]
        channel_all = file['Base{}ORTauTauJetOR4JetsPNet_baseline_{}_genHH_mass'.format(base_extension, args.channels[0])]
        channel_base = file['Base{}_baseline_{}_genHH_mass'.format(base_extension, args.channels[0])]
        channel_base_plus_ttjet = file['Base{}ORTauTauJet_baseline_{}_genHH_mass'.format(base_extension, args.channels[0])]
        channel_base_plus_quadJetPNet = file['Base{}OR4JetsPNet_baseline_{}_genHH_mass'.format(base_extension, args.channels[0])]
        all_channel = file['All_baseline_{}_genHH_mass'.format(args.channels[0])]
        # channel_base_plus_ttjet = file['NoBaseMuTTJet_baseline_{}_genHH_mass'.format(args.channels[0])]
        # channel_base_plus_quadJetDeep = file['NoBaseMu4JetsDeepJet_baseline_{}_genHH_mass'.format(args.channels[0])]
        # channel_all = file['Base{}ORTauTauJetOR4JetsPNet_baseline_{}_genHH_mass'.format(base_extension, args.channels[0])]
        
        # channel_base_plus_Mu50 = file['NoBaseMuMu50_baseline_{}'.format(args.channels[0])]
        # channel_base_plus_Ele28 = file['NoBaseEle28_baseline_{}'.format(args.channels[0])]
        # if args.channels[0] == "tautau":
        #     channel_all = file['NoBaseMuTauMETORTauORTTJet_baseline_{}'.format(args.channels[0])]
        #     base = "DoubleMediumDeepTau"
        #     last_trig = "TauTauJet"
        # if args.channels[0] == "mutau":
        #     channel_all = file['NoBaseMETORTauORMu50_baseline_{}'.format(args.channels[0])]
        #     base = "IsoMu + CrossMuTau"
        #     last_trig = "Mu50"
        # if args.channels[0] == "etau":
        #     channel_all = file['NoBaseMETORTauOREle28_baseline_{}'.format(args.channels[0])]
        ...
        err_base_plus_ttjet = np.sqrt(channel_base_plus_ttjet.values())/ channel_all.values()
        err_base_plus_quadJetPNet = np.sqrt(channel_base_plus_quadJetPNet.values())/ channel_all.values()
        ...
        err_all = np.sqrt(channel_all.values())/ channel_all.values()
        ...
        print("generated additional axis")
        ax2.set_ylabel("Weighted MC events [a.u.]")  # we already handled the x-label with ax1
        ax2.bar(ax, height=all_channel.values(), width=xerr_r * 2, color=petroff6[4], alpha=0.3, zorder=0, label="HHbbtautau")
        print("Initialized bars")
        hep.style.use("CMS")
        ax1.errorbar(ax, (channel_base_plus_ttjet.values() / channel_base.values() - 1) * 100, yerr=err_base_plus_ttjet*100, xerr=[xerr_r, xerr_l], fmt='o', label="Base + TauTauJet", color=petroff6[0], zorder=5)
        ax1.errorbar(ax, (channel_base_plus_quadJetPNet.values() / channel_base.values() - 1) * 100, yerr=err_base_plus_quadJetPNet*100, xerr=[xerr_r, xerr_l], fmt='o', label="Base + QuadJetPNet", color=petroff6[1], zorder=5)
        if args.channels[0] == "tautau":
            # ax1.errorbar(ax, (channel_base_plus_ttjet.values() / channel_base.values()) * 100, yerr=err_base_plus_ttjet*100, xerr=[xerr_r + 10, xerr_l - 10], fmt='o', label="Base + TauTauJet", color=petroff6[2], zorder=5)
            # ax1.errorbar(ax, (channel_base_plus_quadJetPNet.values() / channel_base.values() - 1) * 100, yerr=err_base_plus_quadJetPNet*100, xerr=[xerr_r, xerr_l], fmt='o', label="Base + QuadJetPNet", color=petroff6[1], zorder=5)
            ...
            ax1.errorbar(ax + 10, (channel_all.values() / channel_base.values() - 1) * 100, yerr=err_all*100, xerr=[xerr_r + 10, xerr_l - 10], fmt='o', label="Base + TauTauJet + QuadJetPNet", color=petroff6[5], zorder=5)
        # if args.channels[0] == "mutau":
        ...
        #     # ax1.errorbar(ax + 10, (channel_all.values() / channel_base.values() - 1) * 100, yerr=err_all*100, xerr=[xerr_r + 10, xerr_l - 10], fmt='o', label="Base + TauTauJet + QuadJetPNet", color=petroff6[5], zorder=5)
        # if args.channels[0] == "etau":
            # ax1.errorbar(ax + 10, (channel_base_plus_quadJetPNet.values() / channel_base.values()) * 100, yerr=err_base_plus_quadJetPNet*100, xerr=[xerr_r + 10, xerr_l - 10], fmt='o', label="Base + QuadJetPNet", color=petroff6[4], zorder=5)
            # ax1.errorbar(ax + 10, (channel_all.values() / channel_base.values() - 1) * 100, yerr=err_all*100, xerr=[xerr_r + 10, xerr_l - 10], fmt='o', label="Base + TauTauJet + QuadJetPNet", color=petroff6[5], zorder=5)
        ...
        # print("Total Gain: {} +/- {}".format(round(channel_all.values().sum()/channel_base.values().sum() * 100, 2), round(err_tot_all * 100, 2)))
        hep.cms.label(lumi="9.8", com="13.6")
        plt.savefig("gains_{}_11p5_ttjet-only_pf75.png".format(args.channels[0]))
        # print(ratio.axes[0])
        return
        for i in vars:
            channel_base_plus_ttjet = file['Base{}ORTauTauJet_baseline_{}_{}'.format(base_extension, args.channels[0], i)]
            channel_only_quadJetPNet = file['NoBase{}ORNoTauTauJet4JetsPNet_baseline_{}_{}'.format(base_extension, args.channels[0], i)]
            channel_all = file['Base{}ORTauTauJetOR4JetsPNet_baseline_{}_{}'.format(base_extension, args.channels[0], i)]
            err_base_plus_ttjet = np.sqrt(channel_base_plus_ttjet.values())
            err_only_quadJetPNet = np.sqrt(channel_only_quadJetPNet.values())
            # err_base_plus_ttjet = np.sqrt( (1/channel_base.values() * channel_base_plus_ttjet.errors()) ** 2 + (channel_base_plus_ttjet.values()/(channel_base.values() ** 2) * channel_base.errors()) ** 2 )
            # err_base_plus_quadJetDeep = np.sqrt( (1/channel_base.values() * channel_base_plus_quadJetDeep.errors()) ** 2 + (channel_base_plus_quadJetDeep.values()/(channel_base.values() ** 2) * channel_base.errors()) ** 2 )
            # err_base_plus_Mu50 = np.sqrt( (1/channel_base.values() * channel_base_plus_Mu50.errors()) ** 2 + (channel_base_plus_Mu50.values()/(channel_base.values() ** 2) * channel_base.errors()) ** 2 )
            # err_base_plus_Ele28 = np.sqrt( (1/channel_base.values() * channel_base_plus_Ele28.errors()) ** 2 + (channel_base_plus_Ele28.values()/(channel_base.values() ** 2) * channel_base.errors()) ** 2 )
            ...
            ax1.errorbar(ax - xerr_r * 0.2, channel_base_plus_ttjet.values(), yerr=err_base_plus_ttjet, xerr=[xerr_r * 0.8 , xerr_l * 1.2], fmt='o', label="Base + TauTauJet", color=petroff6[3], zorder=5)
            ax1.errorbar(ax + xerr_r * 0.2, channel_only_quadJetPNet.values(), yerr=err_only_quadJetPNet, xerr=[xerr_r * 1.2, xerr_l * 0.8], fmt='o', label="QuadJetPNet", color=petroff6[2], zorder=5)
            ...
            ax1.set_ylabel("Weighted MC events [a.u.]")
            lines, labels = ax1.get_legend_handles_labels()
            lines2, labels2 = ax2.get_legend_handles_labels()
            plt.legend(lines + lines2, labels + labels2, loc=4,  bbox_to_anchor=(1, 0.1))
            # print("Total Gain: {} +/- {}".format(round(channel_all.values().sum()/channel_base.values().sum() * 100, 2), round(err_tot_all * 100, 2)))
            hep.cms.label(lumi="9.8", com="13.6")
            plt.savefig("gains_{}_quadJet_preEE_10p20_withOffline_{}.png".format(args.channels[0], i))
    return 
