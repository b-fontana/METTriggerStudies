...
for chn in channels:
    ...
    for ktd, vtd in table_dir.items():
        y[chn][ktd] = []
        d = Path(base_dir) / main_dir / chn / vtd
        with open(Path(d) / 'table.csv') as f:
            reader = csv.reader(f, delimiter=',', quotechar='|')
            next(reader, None) #ignore header line
            for sample, frac in reader:
                y[chn][ktd].append(float(frac))

for chn in channels:
    y[chn]['add_met_tau'] = []
    for elem1, elem2 in zip(y[chn]['met'], y[chn]['met_tau']):
        y[chn]['add_met_tau'].append(elem1+elem2)
...
for ichn,chn in enumerate(channels):
    for itd,td in enumerate(('met', 'tau')):
        p1.circle(linear_x, y[chn][td], color=colors[ichn], fill_alpha=1., **opt_points)
        p1.line(linear_x, y[chn][td], color=colors[ichn], line_dash=styles[itd],
                legend_label=chn+legends[td], **opt_line)
    for itd,td in enumerate(('met', 'add_met_tau')):
        p2.circle(linear_x, y[chn][td], color=colors[ichn], fill_alpha=1., **opt_points)
        p2.line(linear_x, y[chn][td], color=colors[ichn], line_dash=styles[itd],
                legend_label=chn+legends[td], **opt_line)
